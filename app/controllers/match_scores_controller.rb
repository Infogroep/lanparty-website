class MatchScoresController < ApplicationController
	...
	def create
		...
		respond_to do |format|
			if @match_score.save
				format.html { redirect_to match_scores_url, flash: { info: 'Match score was successfully created.' } }
	...
	def update
		...
		respond_to do |format|
			if @match_score.update_attributes(params[:match_score])
				format.html { redirect_to match_scores_url, flash: { info: 'Match score was successfully updated.' } }
